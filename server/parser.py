...
from MyLib import InputInformation # Класс для удобного обмена информацией между функциями
...
def xml_parse(content: str):
    ...
    # Структура xml файла строго описана в прилагаемой pdf интсрукции, обращаемся к элементам 
    # по числовому индексу, так как они упорядочены
    ...
    try:
        ...
        print("CONTENT ERROR: Wrong number of records in field cpu_load_limit", 
              file=sys.stderr)
        return InputInformation(False)
    ...
    try:
        ...
        for element in root[4]:
            ...
            if (prog2, prog1) in links or (prog1, prog2) in links:
                print("CONTENT ERROR: Pair of programs (%d, %d) recorded twice. Second time found at pair number %d" 
                      %(prog1, prog2, cnt))
                return InputInformation(False)
            else:
    ...
    return InputInformation(True).set(number_of_cpu, cpu_load_limits, 
                                      number_of_programs, programs_load, links)
